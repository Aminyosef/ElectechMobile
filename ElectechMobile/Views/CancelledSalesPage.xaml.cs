...
public partial class CancelledSalesPage : ContentPage
{
	public CancelledSalesPage()
	{
		InitializeComponent();
    ...
    public async void InitCustomers()
    {
        ...
        try
        {
            ...
            sales = await Salservice.GetAllc();
    ...
    {
        dataGrid.ItemsSource = string.IsNullOrEmpty(e.NewTextValue) ? sales : await Salservice.Searchc(e.NewTextValue); //sales.Where(x => x.custName.Contains(text ?? x.custName, StringComparison.OrdinalIgnoreCase) || x.id.ToString().Contains(text ?? x.id.ToString(), StringComparison.OrdinalIgnoreCase));
    }
    ...
    private async void dataGrid_SelectionChanged(object sender, Syncfusion.Maui.DataGrid.DataGridSelectionChangedEventArgs e)
    {
        try
        {
            ...
            if (selectedRow is sal selectedSal)
            {
                ...
                await Navigation.PushAsync(new OneSaleCPage(selectedSal.id));
            }
            else
            {
